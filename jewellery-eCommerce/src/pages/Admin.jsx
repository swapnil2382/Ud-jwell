...
const Admin = ({ user }) => {
  ...
  const categoryOptions = ['ring', 'chain', 'stud', 'aaram', 'bangle', 'bracelet', 'coin', 'pendant'];
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      setError(null);
      setFormData({ name: '', description: '', category: '', metal: '', gender: '', occasion: '', purity: '', image: '' });
    } catch (err) {
  ...
  const handleUpdateCustomer = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/users/${customerFormData.userId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const data = await response.json();
      setCustomers(customers.map(c => c._id === customerFormData.userId ? data.data : c));
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {activeSection === 'profile' && (
          <ProfileSection user={user} products={products} customers={customers} />
        )}
        {activeSection === 'add' && (
          <AddProductSection 
            formData={formData}
